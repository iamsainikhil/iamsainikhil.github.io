...
export class CommonService {
  ...
  getCollectionData(collectionName: string, opts?: any) {
    ...
    if (query.operator === null) {
      ...
      if (query.order) {
        collection = this.afs.collection(collectionName, ref =>
          ref
            .where(query.complexField, query.operator, query.value)
            .orderBy(query.field, query.reverse ? "desc" : "asc")
            .limit(query.limit)
        );
      } else {
        collection = this.afs.collection(collectionName, ref =>
          ref.where(query.field, query.operator, query.value).limit(query.limit)
        );
      }
    }
  ...
  getSubCollectionData(
    ...
    const subCollection = this.afs.collection(
